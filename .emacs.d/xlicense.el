...
(defvar license-keywords-alist '(("@author@" . copyright-holder)
                                 ("@email@" . copyright-holder-mail)
                                 ("@year@" . (lambda ()
...
(defun insert-license (&optional type)
  ...
  (let ((text (create-license
	       (or type (intern (completing-read "Choose a license type: "
						 license-types nil t default-license)))
               t
	       ;;(read-from-minibuffer "Summary (short description): ")
	       )))
